...
public class CustomExpression implements Cloneable {
	...
	public void setExpression(String expression) {
		...
		// MODIFIED CODE HERE log.info("Built expression " + expression);
	}
	...
	public boolean checkSymbol() {
		...
		if (symbols.contains(symbol.trim())) {
			...
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Symbol " + symbol + " already exists, found " + symbols.get(index));
			return false;
		}
	...
	public boolean checkName() {
		...
		if (names.contains(name.trim())) {
			...
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Name " + name + " already exists, found " + names.get(index));
			return false;
		}
	...
	public boolean checkExpression() {
		...
		for (char c : expression.toCharArray()) {
			...
			case ':':
				if (square <= 0) {
					// MODIFIED CODE HERE log.info(Markers.USER_MARKER, ": found outside range expression");
					return false;
				}
				else
					break;
		...
		if (round != 0 || square != 0) {
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Expression has unballanced brackets");
			return false;
		}
		...
		*/
		...
		try {
			...
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Custom expression " + this.toString() + " invalid : " + e.toString());
			return false;
		}
	...
	protected Calculable buildExpression(ExpressionBuilder b) {
		...
		try {
			...
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Unknown function. Could not build custom expression " + this.toString());
			return null;
		} catch (UnparsableExpressionException e1) {
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Unparsable expression. Could not build custom expression " + this.toString() + ". " + e1.getMessage());
			return null;
		}
	...
	public Variable evaluate(SimulationStatus status) {
		...
		try {
			...
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Unable to calculate expression " + this.expression + " due to empty stack exception");
		}
	...
	public FlightDataType getType() {
		...
		if (ug == null) {
			// MODIFIED CODE HERE log.debug("SI unit not found for " + unit + " in expression " + toString() + ". Making a new fixed unit.");
		...
		//// MODIFIED CODE HERE log.debug(this.getClass().getSimpleName()+" returned type "+type.getName()+" (" + type.getSymbol() + ")" );
	...
	public void addToDocument() {
		...
		if (!expressions.isEmpty()) {
			...
			if (expressions.contains(this)) {
				// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Expression already in document. This unit : " + this.getUnit() + ", existing unit : " + expressions.get(0).getUnit());
				return;
			}
		}
		
		if (this.checkAll()) {
			// MODIFIED CODE HERE log.info(Markers.USER_MARKER, "Custom expression added to rocket document");
	...
	public void overwrite(CustomExpression newExpression) {
		if (!doc.getCustomExpressions().contains(this))
			...
			// MODIFIED CODE HERE log.debug("Overwriting custom expression already in document");
		}
	}
